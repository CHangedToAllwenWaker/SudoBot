...
if chat_info == 'block' then 
 api.sendMessage(msg.chat.id, '*Oh No!*\n_You Are Blocked :(_', true) 
else
 db:hset(hash, user_id, 'true')
 api.sendMessage(msg.chat.id, '*- Chat Started*\n_Send Your Message For Sudo Team_', true) 
 end
 end
...
api.sendMessage(user_id, '_You are UnBlocked_', true) 
    ...
    if msg.reply and msg.reply.forward_from and msg.chat.type == 'group' and msg.chat.id == -139110802 then
